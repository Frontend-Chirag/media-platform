...
export async function POST(request: NextRequest) {
    try {
        ...
        // Find the user by ID and update the gender
        ...
        return NextResponse.json({ message: 'Gender Updated Successfully' }, { status: 200 });
    } catch (error) {
        // Log any errors to the console
        console.log(error);

        // Return an error message with a 500 status code
        return NextResponse.json({ message: 'Failed to update gender' }, { status: 500 });
    }
}
