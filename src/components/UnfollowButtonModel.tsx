...
const UnfollowButtonModel = () => {
    ...
    return (
        <div className={`${isOpenUnFollow ? 'flex' : 'hidden'} w-full  h-full z-[99] justify-center items-center absolute left-0 top-0 bg-[#000000b2]`}>
            <div className='flex  w-[350px] h-[150px] flex-col overflow-hidden justify-center items-center dark:bg-neutral-900 bg-white rounded-xl'>
                <p className='dark:text-white text-black text-sm  border-b-[1px] border-neutral-600 w-full h-full flex justify-center items-center '>
                    Are you sure you want to unfollow this user?
                </p>
                <button
                    onClick={handleUnFollow}
                    className=' dark:hover:bg-neutral-800 hover:bg-gray-200 border-b-[1px] border-neutral-600 text-[#2f8bfc] w-full h-full flex justify-center items-center'>
                    {isLoading
                        ? <ClipLoader color='#54535364' size={14} loading={true} />
                        : 'Unfollow'
                    }
                </button>
                <button
                    onClick={onCloseUnFollow}
                    className='dark:hover:bg-neutral-800 hover:bg-gray-200 text-red-800  w-full h-full flex justify-center items-center'>
                    Cancel
                </button>
            </div>
        </div>
